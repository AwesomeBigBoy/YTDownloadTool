...
> **NativeAOT removed — design deviation from spec §2.** WPF and NativeAOT are mutually exclusive in .NET 8 (SDK hard-errors on `UseWpf=true + PublishTrimmed=true`; WPF's reflection-heavy XAML/binding subsystem isn't trim-safe; see https://aka.ms/dotnet-illink/wpf). The spec promised ~15 MB exe; reality with self-contained single-file compressed publish is ~50–80 MB. All other spec guarantees (AD trust via pure MS tech, no WebView2 dependency, portable layout) remain intact. Microsoft.NET.Sdk source of the error: `Microsoft.NET.RuntimeIdentifierInference.targets:254-258`.
>
