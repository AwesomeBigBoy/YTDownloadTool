...
";
    ...
    // Fulcio intermediate CA (PEM). Fulcio's short-lived leaf certs are issued by
    // this intermediate, NOT directly by the root. X509Chain.Build needs both the
    // root AND the intermediate in the trust path or it returns
    // PartialChain / UntrustedRoot ("憑證鏈結無法建立於受信任的根授權").
    // Source: https://github.com/sigstore/root-signing/blob/main/targets/fulcio_intermediate_v1.crt.pem
    public const string FulcioIntermediatePem = @"-----BEGIN CERTIFICATE-----
