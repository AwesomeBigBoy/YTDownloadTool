...
public static class ErrorMapper
{
    ...
    private static readonly Rule[] Rules =
    {
        ...
        new(ErrorCategory.NetworkError,      "E-TIMEOUT01",
            "解析網址逾時（無法連到 YouTube）。可能原因：DNS 解析失敗、目前網路封鎖 *.youtube.com / *.googlevideo.com、HTTPS 設定問題、或網路斷線。請改用其他網路後重試。", true,
    ...
    public static MappedError Map(string stderr)
    {
        ...
        // v1.2.4: detect the OpenSSL SECLEVEL rejection BEFORE the generic SSL pattern,
        // because the message it deserves is action-specific (point at the AllowUntrusted
        // setting), not the generic "verify failed" message.
