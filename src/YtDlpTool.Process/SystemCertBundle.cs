...
///
/// Why: managed environments commonly run an SSL-inspection proxy ((HTTPS inspection product) / (HTTPS inspection product) /
/// (HTTPS inspection product) / web gateway) that re-signs HTTPS traffic with an enterprise
